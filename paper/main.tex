...
%% ── AEA Draft Spacing (1.5 = "double" for review) ───────────
\draftSpacing{1.5}

%% ── Natbib (per AEA template instructions) ───────────────────
\usepackage{natbib}

%% ── Math (AEA.cls loads amsmath[leqno]; add extra symbols) ──
%% AEA.cls defines its own proof env; clear it so amsthm can define the standard one
\let\proof\relax
\let\endproof\relax
...
%% Figure/table reference shorthand
...
%% ── Title Block (inside document per AEA format) ─────────────
\title{Monetary Policy in Fractured Markets: A Unified HANK-IO
       Framework with Structural Monopsony and Geoeconomic
       Fragmentation}

\shortTitle{Monetary Policy in Fractured Markets}
...
%% ── JEL Classification ───────────────────────────────────────
\JEL{E12, E24, E52, F41, J42}

%% ── Abstract (before \maketitle per AEA format) ─────────────
\begin{abstract}
