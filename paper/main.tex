...
%% ── Typography ───────────────────────────────────────────────
\setstretch{1.25}          % AEA-style 1.25 line spacing
...
\title{\textbf{Monetary Policy in Fractured Markets:} \\[6pt]
       A Unified {HANK-IO} Framework with Structural Monopsony \\
       and Geoeconomic Fragmentation\thanks{%
         I thank seminar participants at the Federal Reserve Board,
         European Central Bank, NBER EF\&G Program,
         Society for Economic Dynamics, and
         the Princeton Macro lunch for valuable comments.
         All errors are my own.}}

\author{Brendan Loalt\thanks{%
  Department of Economics.
  Email: \href{mailto:brendanloalt@gmail.com}{brendanloalt@gmail.com}.}}
...
\input{sections/04_solution_method}

\input{sections/05_analytical_results}

\input{sections/06_quantitative}
...
\begin{figure}[p]
  \centering
  \includegraphics[width=\linewidth]{figures/fig8_network_graph.pdf}
  \caption{\textbf{Multi-Region IO Network: Integrated vs.\ Fragmented.}
    Stylized visualization of the 10-node, 2-bloc trade network under
    integration ($\phi=0$, left) and high fragmentation ($\phi=0.30$,
    right). Node color denotes geopolitical bloc (blue: Western,
    red: Eastern). Solid edges are within-bloc flows; dashed edges
    are cross-bloc flows (opacity proportional to IO coefficient,
    which falls under fragmentation). The dashed vertical line marks
    the geopolitical fault line that intensifies under fragmentation.
    Under high $\phi$, cross-bloc edges thin dramatically, domestic
    upstream nodes gain centrality, and the network becomes less
    diversified.}
  \label{fig:fig8}
\end{figure}
