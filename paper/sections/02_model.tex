\section{Model}
\label{sec:model}

The economy consists of $S$ sectors, $R$ trade regions partitioned into
$B$ geopolitical blocs, a representative household, a continuum of firms
with pricing and wage-setting power, and a monetary authority.
Time is discrete. The aggregate state at date $t$ is
$\{z_{st}, \phi_t\}$: sectoral productivities and the fragmentation premium.

\subsection{A Two-Sector Analytical Model of the Core Non-Linearity}
\label{sec:toy_model}

Before presenting the full model, we establish the core
IO$\times$Monopsony non-linearity in a static, two-sector economy that
admits closed-form analysis. The full quantitative model generalizes every
dimension of this setup; the mechanism is identical.

\paragraph{Setup.}
The economy has two sectors: Upstream $U$ (goods) and Downstream $D$
(services). Each has a continuum of firms facing an upward-sloping
firm-level labor supply curve with sector-specific elasticity
$\varepsilon_s > 0$, where $\varepsilon_U < \varepsilon_D$---goods sectors
are more monopsonistic than service sectors, following
\citet{dube2020monopsony}. Production uses labor only:
$y_s = z_s\,n_s^{\alpha_s}$, so the marginal product of labor is
$\mathrm{MPL}_s = \alpha_s z_s n_s^{\alpha_s-1}$. The Downstream sector
uses Upstream output as an intermediate input with IO cost share
$\Omega \in (0,1)$.

\paragraph{Wage markdown and sector-level labor income.}
Optimal wage-setting under monopsony yields
$w_s = \mu_s^w \cdot \mathrm{MPL}_s$ where $\mu_s^w \equiv
\varepsilon_s/(\varepsilon_s+1) \in (0,1)$ is the sector-specific markdown.
When unemployment rises, workers' outside options deteriorate and
$\varepsilon_s$ falls by $\gamma\,\Delta u$, so the markdown deepens by:
\begin{equation}
  \Delta\mu_s^w \approx -\frac{\gamma}{(\varepsilon_s+1)^2}\,\Delta u
  \equiv -\delta_s\,\Delta u, \qquad \delta_s \equiv
  \frac{\gamma}{(\varepsilon_s+1)^2}.
  \label{eq:delta_s}
\end{equation}
The coefficient $\delta_s$ is \emph{decreasing} in $\varepsilon_s$: more
monopsonistic sectors (lower $\varepsilon_s$) have a \emph{larger}
markdown response per unit unemployment.

\paragraph{Domar weights and fragmentation.}
In this two-sector economy, Domar weights are:
$\lambda_U = (1+\Omega)/(1+\Omega(1-\alpha_U))$ and
$\lambda_D = 1/(1+\Omega(1-\alpha_U))$.\footnote{These follow from
the standard Leontief identity $\mathbf{X} = (\mathbf{I} -
\boldsymbol{\Omega}^\top)^{-1}\mathbf{F}$; see \citet{baqaee2019}.}
When a cross-bloc fragmentation shock $d\phi > 0$ raises the cost of
foreign $U$-substitutes, domestic Upstream production expands:
\begin{equation}
  \frac{d\lambda_U}{d\phi} = A > 0, \qquad
  \frac{d\lambda_D}{d\phi} = -A,
  \label{eq:domar_shift}
\end{equation}
where $A = \eta\,\lambda_U\lambda_D\,\Omega/\bar\tau > 0$ is the
Armington first-order effect (see Supplemental Appendix).

\paragraph{The core non-linearity.}
Define the \emph{aggregate wage-suppression index}---the expected markdown
deepening per unit unemployment, weighted by sectors' share of economic
activity:
\begin{equation}
  W_{\text{agg}} \equiv \lambda_U\,\delta_U + \lambda_D\,\delta_D.
  \label{eq:W_agg}
\end{equation}
This is the cross-sectional average of the markdown coefficient, weighted
by Domar shares. Its response to a fragmentation shock is:
\begin{equation}
  \boxed{\frac{dW_{\text{agg}}}{d\phi} = A\!\left(\delta_U - \delta_D\right)
  = A\,\gamma\!\left[\frac{1}{(\varepsilon_U+1)^2} -
  \frac{1}{(\varepsilon_D+1)^2}\right] > 0.}
  \label{eq:core_nonlinearity}
\end{equation}
This is strictly positive \emph{if and only if} $\varepsilon_U < \varepsilon_D$.

\begin{proposition}[Core Non-Linearity: Analytical]\label{prop:toy}
  In the two-sector model:
  \begin{enumerate}[(i)]
    \item An IO-only model $(\varepsilon_U = \varepsilon_D = \bar\varepsilon)$
      predicts $dW_{\emph{agg}}/d\phi = 0$: fragmentation shifts Domar
      weights but generates no differential wage compression.
    \item A monopsony-only model (no IO linkages, $A = 0$) predicts
      $dW_{\emph{agg}}/d\phi = 0$: markdowns deepen but the
      sector-composition of activity is unaffected.
    \item The joint model predicts $dW_{\emph{agg}}/d\phi > 0$: Domar
      reallocation into high-monopsony upstream sectors amplifies wage
      suppression super-proportionally, at rate
      $A\gamma[(\varepsilon_U+1)^{-2} - (\varepsilon_D+1)^{-2}]$.
  \end{enumerate}
  The amplification factor relative to a model with uniform monopsony is:
  \begin{equation}
    \mathcal{A} \equiv \frac{W_{\emph{agg}}^{\emph{joint}}}
    {W_{\emph{agg}}^{\emph{uniform}}} =
    \frac{\lambda_U(\varepsilon_U+1)^{-2} + \lambda_D(\varepsilon_D+1)^{-2}}
    {(\bar\varepsilon+1)^{-2}} > 1.
    \label{eq:amp_factor}
  \end{equation}
\end{proposition}

\paragraph{Calibrated magnitudes.}
With $\varepsilon_U = 2.5$, $\varepsilon_D = 5.5$, $\bar\varepsilon = 4$
(from \citealt{dube2020monopsony}), the amplification factor is:
$\mathcal{A} = [(2.5+1)^{-2} + (5.5+1)^{-2}]/2 \div (4+1)^{-2} = [0.082
+ 0.024]/2 \div 0.040 = 0.053/0.040 \approx 1.33$. The $3.4\times$ ratio
of goods-sector to services-sector wage compression ($\delta_U/\delta_D
= (5.5+1)^2/(2.5+1)^2 \approx 3.4$) is a corollary. Both figures carry
directly into the full quantitative model.

\subsection{Households}
\label{sec:households}

A representative household maximizes:
\begin{equation}
  \max_{\{c_t, a_{t+1}, n_t\}} \mathbb{E}_0 \sum_{t=0}^{\infty} \beta^t
  \left[\frac{c_t^{1-\sigma}}{1-\sigma} - \chi \frac{n_t^{1+\varphi}}{1+\varphi}\right]
  \label{eq:hh_objective}
\end{equation}
subject to the budget constraint:
\begin{equation}
  c_t + a_{t+1} = (1 + r_t)\,a_t + w_t\,n_t + T_t
  \label{eq:bc}
\end{equation}
where $r_t$ is the real interest rate, $w_t$ is the effective wage
(reflecting the monopsony markdown), and $T_t$ are lump-sum transfers.
The first-order conditions are the standard Euler equation and
intratemporal labor supply condition:
\begin{align}
  c_t^{-\sigma} &= \beta(1+r_t)\,\mathbb{E}_t[c_{t+1}^{-\sigma}]
  \label{eq:euler} \\
  \chi\,n_t^{\varphi} &= w_t\,c_t^{-\sigma}.
  \label{eq:labor_supply_hh}
\end{align}

\subsection{Firms}
\label{sec:firms}

Firm $j$ in sector $s$, region $r$ produces output $y_{jt}$ using labor and
intermediate inputs.

\subsubsection{Production Technology}

Output is produced according to a nested CES technology:
\begin{equation}
  y_{jt} = z_{jt} \left[\alpha_n^{1/\theta} n_{jt}^{(\theta-1)/\theta} +
  \alpha_m^{1/\theta} M_{jt}^{(\theta-1)/\theta}\right]^{\theta/(\theta-1)}
  \label{eq:production}
\end{equation}
where $z_{jt}$ is idiosyncratic TFP, $n_{jt}$ is labor input, and
$M_{jt}$ is a CES aggregate of intermediate inputs from all sector-region
pairs:
\begin{equation}
  M_{jt} = \left[\sum_{s',r'} \omega_{s'r'}^{1/\eta}
  \left(\frac{m_{jt}^{s'r'}}{\tau_t^{rr'}}\right)^{(\eta-1)/\eta}
  \right]^{\eta/(\eta-1)}.
  \label{eq:composite_M}
\end{equation}
The term $\tau_t^{rr'} \geq 1$ is the iceberg trade cost between regions
$r$ and $r'$. The cost share parameters satisfy
$\alpha_n + \alpha_m = 1$. The elasticity $\theta$ governs substitution
between labor and intermediates; $\eta$ is the Armington elasticity across
source regions.

\subsubsection{Monopsony in Labor Markets}

Standard NK models assume firms are price-takers in the labor market. We
replace this assumption with an \emph{upward-sloping firm-level labor supply
curve}, which is the defining feature of oligopsonistic labor markets
\citep{manning2011, card2018}.

Firm $j$'s labor supply curve is:
\begin{equation}
  n_{jt} = \left(\frac{w_{jt}}{\bar{w}_t(\ell)}\right)^{\varepsilon_t}
  \bar{N}_t(\ell)
  \label{eq:labor_supply}
\end{equation}
where $\varepsilon_t > 0$ is the \emph{firm-level labor supply elasticity},
$\bar{w}_t(\ell)$ is the market-wide wage for labor type $\ell$, and
$\bar{N}_t(\ell)$ is aggregate labor supply of that type.

When $\varepsilon \to \infty$, equation \eqref{eq:labor_supply} delivers
perfect competition: the firm faces a perfectly elastic labor supply curve at
the market wage. When $\varepsilon$ is finite, the firm internalizes that
raising its wage attracts more workers, implying wage-setting power.

\paragraph{Microfoundation: directed search and the Beveridge flow balance.}
\label{sec:micro_search}
The countercyclical elasticity $\varepsilon_t$ is not assumed but derived
from search-and-matching primitives following \citet{manning2003} and
\citet{berger2022monopsony}. We sketch the derivation here; the full
derivation is in the Supplemental Appendix.

Each employed worker receives outside wage offers at Poisson rate $\lambda_e$.
At firm $j$ posting wage $w_j$, a worker quits if an outside offer $w' > w_j$
arrives. In directed search, the offer acceptance rate depends on the firm's
wage relative to the market: quit rate $= \lambda_e \cdot s(w_j/\bar w_t)$
where $s(\cdot)$ is increasing. The firm's steady-state employment satisfies
a flow balance between inflows (via the job-finding rate $f(\theta_t)$,
$\theta_t$ being market tightness) and outflows:
\begin{equation}
  \underbrace{\delta(1-u_t)}_{\text{exogenous separations}} +
  \underbrace{\lambda_e(1-u_t)}_{\text{poaching}} = f(\theta_t)\,u_t
  \label{eq:beveridge}
\end{equation}
Solving \eqref{eq:beveridge} for the job-finding rate and noting that each
worker's threat point---the value of quitting---falls when $f(\theta_t)$
falls (fewer offers arrive), the firm's optimal wage must rise by less per
unit employment to retain workers. This is the monopsony mechanism:
\begin{equation}
  \varepsilon_t = \varepsilon_\kappa\!\left[1 +
  \frac{\lambda_e(1-u_t)}{f(\theta_t)\,u_t}\right]
  = \varepsilon_\kappa\!\left[1 + \frac{\lambda_e(1-u_t)}{\delta(1-u_t)+\lambda_e(1-u_t)}\right]
  \label{eq:epsilon_exact}
\end{equation}
where $\varepsilon_\kappa > 0$ is a structural elasticity tied to the
wage-posting distribution. Equation \eqref{eq:epsilon_exact} is strictly
decreasing in unemployment: as $u_t$ rises, the Beveridge flow balance
\eqref{eq:beveridge} implies $f(\theta_t)$ falls, workers receive fewer
outside offers, and firms face a more inelastic labor supply. A first-order
Taylor expansion of \eqref{eq:epsilon_exact} around steady state $\bar u$
yields the linear approximation used in the quantitative model:
\begin{equation}
  \varepsilon_t \approx \bar{\varepsilon} - \gamma\,(u_t - \bar{u}),
  \qquad
  \gamma \equiv \frac{\varepsilon_\kappa\,\lambda_e}{\bar{u}^2} > 0,
  \qquad \varepsilon_t \geq \varepsilon_{\min} = 0.5
  \label{eq:epsilon_cyclical}
\end{equation}
The formula $\gamma = \varepsilon_\kappa\lambda_e/\bar u^2 > 0$ is derived
from the search primitives, not assumed. It is calibrated to $\gamma = 0.8$
to match the observed labor share cyclicality ($-0.032$pp per 1pp rise in
$u_t$, \citealt{rinz2022labor}), with an independent micro-panel estimate
$\hat\gamma_{\text{micro}} = 0.75$ (SE 0.12) from the QCEW+Lightcast data
described in Appendix~\ref{app:gamma_estimation}.

\paragraph{Wage-setting.}
Profit-maximizing wage-setting by firm $j$ yields the \emph{wage markdown
condition}:
\begin{equation}
  w_{jt}^* = \underbrace{\frac{\varepsilon_t}{\varepsilon_t + 1}}_{\equiv\,\mu_t^w} \cdot
  \mathrm{MPL}_{jt}
  \label{eq:wage_markdown}
\end{equation}
where $\mu_t^w \in (0,1)$ is the \emph{wage markdown}---the ratio of wage to
marginal product. Under perfect competition, $\mu_t^w = 1$. The parameter
$\gamma > 0$ ensures that $\mu_t^w$ is countercyclical: it falls (the markup
deepens) when unemployment rises.

\paragraph{Calibration of $\bar\varepsilon$.}
We set $\bar\varepsilon = 4$, implying a steady-state markdown of
$\mu^w = 4/5 = 0.80$---consistent with the central estimate of
\citet{manning2021monopsony} and the sector-level estimates of
\citet{dube2020monopsony}. This also pins down $\varepsilon_\kappa = \bar\varepsilon/(1+\bar\Psi) \approx 3.8$
following the microfoundation in Section~\ref{sec:micro_search}, given the calibrated $\lambda_e$.

\subsubsection{Price-Setting}

In goods markets, firms face a standard Calvo \citeyearpar{calvo1983} pricing
friction. A fraction $1-\xi$ of firms can optimally reset their price each
period. A firm that resets chooses $P_{jt}^*$ to maximize the present
discounted value of profits, yielding the standard condition:
\begin{equation}
  P_{jt}^* = \frac{\varepsilon_p}{\varepsilon_p - 1} \cdot
  \mathbb{E}_t\sum_{k=0}^{\infty}(\xi\beta)^k\,\frac{\Lambda_{t+k}}{\Lambda_t}
  \cdot \widehat{mc}_{jt+k} \cdot y_{jt+k|t}
  \label{eq:optimal_price}
\end{equation}
where $\varepsilon_p$ is the elasticity of substitution across goods varieties,
$\Lambda_t$ is the stochastic discount factor, and $\widehat{mc}_{jt}$ is
the \emph{true} marginal cost under monopsonistic labor hiring:
\begin{equation}
  \widehat{mc}_{jt} = \frac{w_{jt}}{\mu_t^w \cdot z_{jt} \cdot f_n}
  = \frac{\mathrm{MPL}_{jt}}{z_{jt} \cdot f_n}.
  \label{eq:true_mc}
\end{equation}
Note that the markdown $\mu_t^w$ cancels in \eqref{eq:true_mc}: the true
marginal cost of an additional unit of output equals the marginal product
value regardless of wage-setting power. However, the markdown \emph{does}
affect real marginal cost dynamics through the general equilibrium link between
employment, productivity, and the markup.

\subsection{The Multi-Region IO Network}
\label{sec:io_network}

\subsubsection{Geopolitical Blocs and Trade Costs}

The world is partitioned into $B = 2$ geopolitical blocs:
$\mathcal{B}_1 = \{\text{US}, \text{EU}, \text{RoW}\}$ and
$\mathcal{B}_2 = \{\text{China}, \text{Emerging Asia}\}$. The iceberg trade
cost between regions $r$ and $r'$ is:
\begin{equation}
  \tau_t^{rr'} = \begin{cases}
    1 & \text{if } r = r' \\
    1 + \delta^{\text{within}} & \text{if } b(r) = b(r') \neq r \\
    1 + \delta^{\text{within}} + \underbrace{\phi_t}_{\text{fragmentation premium}}
      & \text{if } b(r) \neq b(r')
  \end{cases}
  \label{eq:trade_costs}
\end{equation}
The fragmentation premium $\phi_t$ follows an AR(1) process:
\begin{equation}
  \phi_t = \rho_\phi \phi_{t-1} + \sigma_\phi\,\varepsilon_t^\phi,
  \qquad \varepsilon_t^\phi \sim \mathcal{N}(0,1)
  \label{eq:phi_process}
\end{equation}
with high persistence $\rho_\phi = 0.90$, reflecting the slow-moving nature
of geopolitical realignments. The Supplemental Appendix provides a
strategic-game micro-foundation and demonstrates that the AR(1) is the
Nash-equilibrium linearization of a two-bloc tariff game.

\paragraph{Sector-specific regulatory premium.}
The empirical analysis (Section~\ref{sec:ip_frag}) documents that modern
geoeconomic fragmentation operates through IP and regulatory channels in
addition to tariff costs. We extend \eqref{eq:trade_costs} to accommodate
a sector-specific regulatory premium $\psi_{s,t}^{rr'}$:
\begin{equation}
  \tau_{s,t}^{rr'} = \begin{cases}
    1 & r = r' \\
    1 + \delta^{\text{within}} & b(r)=b(r'), r\neq r' \\
    1 + \delta^{\text{within}} + \phi_t + \psi_{s,t}^{rr'} & b(r)\neq b(r')
  \end{cases}
  \label{eq:trade_costs_full}
\end{equation}
In the baseline model, $\psi_{s,t} \equiv 0$ (sector-neutral $\phi_t$ shock).
In the extended model with IP decoupling (Supplemental Appendix
and Section~\ref{sec:ip_frag}), $\psi_{s,t}^{\text{Tech}}$ captures
semiconductor export control shocks. All main results hold under either
specification; the IP extension is used in the broadened fragmentation
exposure index in Section~\ref{sec:data}.

\subsubsection{The Leontief IO Structure}

We represent the global production network as a weighted directed graph
$\mathcal{G} = (\mathcal{V}, \mathcal{E}, \boldsymbol{\Omega})$ where:
\begin{itemize}
  \item Nodes $\mathcal{V}$: sector-region pairs $(s,r)$, with
    $|\mathcal{V}| = S \times R = 10 \times 5 = 50$.
  \item Edges $\mathcal{E}$: input-output linkages between node pairs.
  \item Weights $\Omega_{(s,r)(s',r')} \in [0,1]$: cost share of sector
    $(s',r')$ inputs in the production of sector $(s,r)$.
\end{itemize}

The IO balance system in matrix form is:
\begin{equation}
  \mathbf{X} = \boldsymbol{\Omega}^\top \mathbf{X} + \mathbf{F}
  \label{eq:io_balance}
\end{equation}
where $\mathbf{X} \in \mathbb{R}^{S \times R}$ is the gross output vector and
$\mathbf{F}$ is final demand. Solving \eqref{eq:io_balance}:
\begin{equation}
  \mathbf{X} = (\mathbf{I} - \boldsymbol{\Omega}^\top)^{-1}\mathbf{F}
  \equiv \mathbf{L}\,\mathbf{F}
  \label{eq:leontief}
\end{equation}
where $\mathbf{L}$ is the \emph{Leontief inverse}. Its $(i,j)$ entry gives
the total output of sector $i$ required per unit of final demand in sector $j$,
including all upstream supply chain rounds.

\paragraph{Domar Weights.}
Following \citet{baqaee2019}, the \emph{Domar weight} of sector $(s,r)$ is:
\begin{equation}
  \lambda_{sr} = \frac{P_{sr} X_{sr}}{P \cdot \mathrm{GDP}}
  \label{eq:domar}
\end{equation}
which equals the sector's share of \emph{gross} (not value-added) output in
GDP. Because sectors supply both final goods and intermediates, Domar weights
sum to more than 1: $\sum_{sr} \lambda_{sr} > 1$.

\paragraph{Fragmentation and Domar Weights.}
A key result of the model is that fragmentation reshapes the Domar weight
distribution. Specifically:

\begin{lemma}[Domar Weight Reallocation]
  \label{lemma:domar}
  For any upstream sector $(s,r)$ that primarily supplies cross-bloc
  intermediate goods: $\partial\lambda_{sr}/\partial\phi > 0$.
  For downstream service sectors with limited cross-bloc linkages:
  $\partial\lambda_{sr}/\partial\phi \approx 0$.
\end{lemma}

\begin{proof}
  See Appendix \ref{app:proofs}.
\end{proof}

The intuition is straightforward: when cross-bloc trade costs rise, domestic
firms substitute toward domestic upstream suppliers. This increases the volume
of domestic upstream production relative to GDP, raising upstream Domar
weights. The effect is amplified by the Leontief multiplier, which propagates
first-order substitutions into higher-order input demand changes.

\subsection{Monetary Authority}
\label{sec:monetary}

The central bank sets the nominal interest rate according to a (potentially
augmented) Taylor rule:
\begin{equation}
  i_t = r^* + \pi^* + \phi_\pi(\pi_t - \pi^*) + \phi_y\,\tilde{y}_t
        + \phi_\mu\,\hat\mu_t^w + \phi_\phi\,\hat\phi_t + \nu_t
  \label{eq:taylor}
\end{equation}
where $\tilde{y}_t$ is the output gap, $\hat\mu_t^w \equiv \mu_t^w - \bar\mu^w$
is the wage markdown gap, $\hat\phi_t \equiv \phi_t - \bar\phi$ is the
fragmentation premium deviation from steady state, and $\nu_t$ is an
exogenous monetary policy shock. The \emph{standard} Taylor rule corresponds
to $\phi_\mu = \phi_\phi = 0$; the \emph{augmented} rule allows these
coefficients to differ from zero.

Proposition 3 characterizes the welfare-optimal values of all four
coefficients $(\phi_\pi^*, \phi_y^*, \phi_\mu^*, \phi_\phi^*)$.
