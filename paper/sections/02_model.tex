...
\paragraph{Countercyclical markdown.}
A critical departure from static monopsony models is that we allow
$\varepsilon_t$ to vary over the business cycle. When unemployment is high,
workers' outside options deteriorate and their mobility falls, reducing the
elasticity of their labor supply to any given firm:
\begin{equation}
  \varepsilon_t = \bar{\varepsilon} - \gamma\,(u_t - \bar{u}),
  \qquad \varepsilon_t \geq \varepsilon_{\min} > 0
  \label{eq:epsilon_cyclical}
\end{equation}
where $\bar{\varepsilon}$ is the steady-state elasticity, $\gamma > 0$
captures the cyclical sensitivity, and $u_t$ is the aggregate unemployment
rate. We set $\varepsilon_{\min} = 0.5$ to bound the markdown from above.
...
\citet{dube2020monopsony}. The countercyclical sensitivity $\gamma = 0.8$
is calibrated to match the observed variation in wage growth across recessions
documented by \citet{rinz2022labor}.
