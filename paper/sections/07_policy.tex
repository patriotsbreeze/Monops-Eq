...
\begin{enumerate}
  \item \textbf{Labor market concentration indicator} ($\phi_\mu^* > 0$):
    ...
    Our baseline calibration finds $\phi_\mu^* \approx 0$ for $\sigma_\phi = 0.05$,
    rising as fragmentation variance and monopsony cyclicality increase;
    the sign is positive whenever $\gamma > 0$.
...
\end{enumerate}
A policy that raises $\bar\varepsilon$ from 4 to 8 (moving from the median
U.S.\ labour market to a more competitive benchmark) would reduce $\delta_\mu$
by half and steepen $\kappa^{mono}$ toward $\kappa^{comp}$, lowering the
model-implied sacrifice ratio accordingly. The precise welfare gain requires
knowledge of the counterfactual disinflation path; what the model delivers is
a structural formula linking concentration ($\bar\varepsilon$) to monetary
policy effectiveness ($\kappa^{mono}$).
