...
are $SR^{comp} = 3.21$ and $SR^{mono} = 3.26$. These aggregate magnitudes are
modest at standard calibration but scale substantially with $\gamma$: in
industries with low labour mobility (mining, energy, basic manufacturing),
$\gamma$ is plausibly above 1.0, implying flattening of 2--3\% and a sacrifice
ratio gap of 0.1--0.2 units. More importantly, the cross-sectional
implication is large: the $3.4\times$ ratio of upstream-to-downstream
wage compression derived analytically in Section~\ref{sec:toy_model}
($\delta_U/\delta_D \approx 3.4$) is the primary empirical target
of the Bartik design in Section~\ref{sec:empirical}. The aggregate Phillips
curve flattening and the cross-sectional wage dispersion are two faces of
the same structural parameter $\gamma$ and the same mechanism.
